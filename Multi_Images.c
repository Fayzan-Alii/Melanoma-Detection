...
#define OUTPUT_DIR "outputImages/"
#define NUM_IMAGES 5
...
int main()
{
    ...
    for (i = 0; i < NUM_IMAGES && i < num_files; i++)
    {
        char input_path[1024];
        sprintf(input_path, "%s%s", INPUT_DIR, files[i]);

        // Load the image
        int width, height, channels;
        unsigned char *img = stbi_load(input_path, &width, &height, &channels, STBI_rgb_alpha);
        if (!img)
        {
        ...
        sprintf(output_path, "%soutput_%s", OUTPUT_DIR, files[i]);
        stbi_write_jpg(output_path, width, height, 4, img_grey, 100);
